...
Our ridge regression model was able to predict city park system rankings with relatively strong performance, fitting quite well. After removing the feature `rank_last_time`, which could've potentially introduced data leakage, the model achieved a test score of approximately `{python} round(test_r2*100, 1)`% and showed no signs of underfitting or overfitting, indicating that the available park system characteristics still explain a significant portion (`{python} round(test_r2*100, 1)`%) of the variation in rankings. Looking at @fig-actual_vs_predicted, we can see that the predicted ranks regression line is very close to the 45-degree line $y=x$. Therefore, this suggests that measurable park attributes, such as park size, accessibility, and amenities do contribute meaningfully to how cities are ranked in the ParkScore system. 
...
Finally, sustainability would also be an important feature to incorporate, adding a sustainability ranking, as sustainability for future generations is important in this context.
