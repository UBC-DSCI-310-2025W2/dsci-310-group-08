...
Since our target variable `rank` is a discrete numerical variable ranging from 1 to `{python} y_train_max`, it would be best to use **ridge regression** to build a predictive model. We process the categorical variables `state` and `year` using one hot encoding to convert them into numerical data. We drop `city` as a feature as it perfectly correlates with `state`. We also scale all numerical features to a standardized scale.
...
Before implementing the model on the data, we optimize the regularization hyperparameter $alpha$ through randomized grid search. This tests the model performance using different values of $alpha$ and selects the one that results in the best performance.
...
This model can predict `rank` with an almost perfect accuracy of `{python} round(grid_search.best_score_, 4)`. This seems a bit unrealistic so let's take a look at the coefficients to see if there are any unexpected relationships.
...
```{python}
# hyperparameter tuning
grid_search_new = RandomizedSearchCV(pipe_new, param_distributions=param_grid, random_state=123,
                                    n_iter=100, n_jobs=-1, return_train_score=True)
...
The optimized value of $alpha$ for the final model is `{python} round(grid_search_new.best_params_['ridge__alpha'], 4)`, with a performance score of `{python} round(grid_search_new.best_score_, 4)`.

This looks like a much more realistic result. Now we will fit the model on the training data using `{python} round(grid_search_new.best_params_['ridge__alpha'], 4)` and evaluate its performance on the test set.
...
Finally, the model achieved a test score of `{python} round(final_pipe.score(X_test, y_test), 4)` on the test set.

### Results & Visualizations
The model performed well on the testing data. To better understand the relationships between predictor variables and the target, we examine the model coefficients.
...
```{python}
#| label: tbl-top10-features
#| tbl-cap: Top 10 Features

coef_df = pd.read_csv("../outputs/results/coefficients.csv")
coef_df.head(10)
```

```{python}
#| label: tbl-bottom5-features
#| tbl-cap: Bottom 5 Features

coef_df = pd.read_csv("../outputs/results/coefficients.csv")
coef_df.tail(5)
```

Since a lower rank is better, a positive coefficient predicts a worse rank whereas a negative coefficient predicts a better rank.
- We can see that cities in 2020 and 2019 tend to receive much worse predicted ranks compared to the baseline year, 2021, whereas from 2015-2018, cities tend to rank better. These shifts in ranking across years possibly reflect changes in the ranking methodology or shifts in overall park system scores across years rather than true differences in park quality.
...
![Actual vs Predicted Ranks](../outputs/results/04_actual-vs-predicted.png){#fig-actual_pred width=70%}
