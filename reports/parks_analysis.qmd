...
max_points = int(X_train_summary.loc['max', 'park_pct_city_points'])
mean_min = round(X_train_summary.loc['mean'].min(), 1)
mean_max = round(X_train_summary.loc['mean'].max(), 1)
```

The first thing to note is that every column has `{python} n_rows` values, meaning all of the NA values were successfully removed. Most of the point scales are out of `{python} max_points`, and the average number of points hovers between `{python} mean_min`-`{python} mean_max` points in all categories.
